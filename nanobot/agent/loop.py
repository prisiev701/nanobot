...
class AgentLoop:
    ...
    def __init__(
        ...
        self.agent_model = agent_model or self.model
    ...
    async def _process_message(
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                for tool_call in response.tool_calls:
                    ...
                    self.collector.record_tool_event(
                        ToolEvent(
                            ts=datetime.now().isoformat(),
                            session_id=key,
                            ...
                            error=result[:500] if not tool_ok else None,
                            iteration=iteration,
                        )
                    )
        ...
        session_duration = int((time.monotonic() - session_started) * 1000)
        session_success = iteration < self.max_iterations and final_content is not None
        self.collector.record_session(
            SessionSummary(
                ...
                failure_reason=f"max_iterations ({self.max_iterations})"
                if iteration >= self.max_iterations
                else None,
                channel=msg.channel,
                model=self.model,
